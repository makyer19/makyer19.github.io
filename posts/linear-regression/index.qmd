...
While we won't complete an example of non-linear regression, we will describe what it is. Simply put, any form of regression that is not linear, must then be considered non-linear. Like the plot with the regression line shown above, linear regressions will follow the simply y = mx + b equation. On the other hand, non-linear regressions will include any regression equations that do not follow that pattern. Some basic non-linear regression patterns include power or exponential, a Weibull growth, or a Fourier equation. Everything else about regression is the same, just the non-linearregression equation follows a different pattern than the li
